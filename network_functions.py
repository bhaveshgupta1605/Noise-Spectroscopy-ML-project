####################### Bash script for training the network
BATCH_SIZE=64
EPOCHS=100
INITIAL_LR=0.001
MIN_LR=0.00001
...
def get_model( filter_nb, kernel_size, pool_size, dropout_rate, xtrain_size ):
	...
	"""
	Constructs a 1D convolutional neural network for denoising noise spectroscopy data.

	Parameters:
	filter_nb (int): Number of filters for each convolutional layer.
	kernel_size (int): Size of the convolutional kernel.
	pool_size (int): Size of the max pooling kernel.
	dropout_rate (float): Dropout rate for the dropout layer.
	xtrain_size (int): Length of the input data.

	Returns:
	model: A 1D convolutional neural network model.
	"""
